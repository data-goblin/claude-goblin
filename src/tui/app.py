...
class KPICard(Static):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Label(self.label, classes="kpi-label")
...
class ClaudeGoblinApp(App):
    ...
    def update_kpis(self) -> None:
        ...
        kpi_tokens = self.query_one("#kpi-tokens", KPICard)
        kpi_tokens.value = _fmt(tokens)
        kpi_tokens.refresh()

        kpi_prompts = self.query_one("#kpi-prompts", KPICard)
        kpi_prompts.value = str(prompts)
        kpi_prompts.refresh()

        kpi_sessions = self.query_one("#kpi-sessions", KPICard)
        kpi_sessions.value = str(sessions)
        kpi_sessions.refresh()

        kpi_days = self.query_one("#kpi-days", KPICard)
        kpi_days.value = str(days)
        kpi_days.refresh()
    ...
    def update_activity(self) -> None:
        ...
        lines.append("")
        lines.append(f"Legend: . (none) : * # @ (most)")
